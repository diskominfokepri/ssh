...
class CObjek extends MainPageD {	
	public function onLoad ($param) {
		...
		if (!$this->IsPostBack&&!$this->IsCallBack) {
            if (!isset($_SESSION['currentPageObjek'])||$_SESSION['currentPageObjek']['page_name']!='d.dmaster.Objek') {
                $_SESSION['currentPageObjek']=array('page_name'=>'d.dmaster.Objek','page_num'=>0,'search'=>false,'no_rek4'=>'none');												
			}
            ...
            $this->cmbRincian->Text=$_SESSION['currentPageObjek']['no_rek4'];
            $this->cmbRincian->dataBind();
            $this->labelfiltered->Text=$_SESSION['currentPageObjek']['no_rek4']=='none'?'[none]':'[selected]';
			$this->populateData ();		
		}
	}
    ...
    public function filterRincian ($sender,$param) {
        ...
        $_SESSION['currentPageObjek']['no_rek4']=$no_rek4; 
        $this->labelfiltered->Text=$no_rek4=='none'?'[none]':'[selected]';
        $this->populateData();
    }
	protected function populateData ($search=false) {
        $no_rek4=$_SESSION['currentPageObjek']['no_rek4'];
        $str_rek=$no_rek4=='none'?'':" AND no_rek4='$no_rek4'";
        if ($search) {
            $str = "SELECT rek5.no_rek5,rek5.nama_rek5,rek5.merek,s.nama_satuan FROM v_rekening rek5 LEFT JOIN satuan s ON (s.id_satuan=rek5.id_satuan) WHERE rek5.id_tipe=1";
            $txtsearch=addslashes($this->txtKriteria->Text);
            switch ($this->cmbKriteria->Text) {                
                case 'kode' :
                    $clausa=" AND no_rek5 LIKE '%$txtsearch%'";
                    $jumlah_baris=$this->DB->getCountRowsOfTable ("v_rekening WHERE id_tipe=1 AND $clausa",'no_rek5');
                    $str = "$str $clausa";
                break;
                case 'nama' :
           ...
           $str = "SELECT rek5.no_rek5,rek5.nama_rek5,rek5.merek,s.nama_satuan FROM v_rekening rek5 LEFT JOIN satuan s ON (s.id_satuan=rek5.id_satuan) WHERE rek5.id_tipe=1$str_rek";
	...
	public function deleteRecord ($sender,$param) {
		$id=$this->getDataKeyField($sender,$this->RepeaterS);
        if ($this->DB->checkRecordIsExist('rekening','uraian_m',$id)) {
        ...
        }elseif($this->DB->checkRecordIsExist('rekening','uraian_p',$id)){
            $this->Pengguna->insertNewActivity($this->Page->getPagePath(),"Tidak bisa menghapus data master Rekening ($id) karena sedang digunakan di Uraian.");
            $this->labelErrorMessage->Text="Tidak bisa menghapus data master Rekening Objek dengan ID ($id) karena sedang digunakan di Uraian Perubahan";
            $this->DialogErrorMessage->Open();
        }else{
