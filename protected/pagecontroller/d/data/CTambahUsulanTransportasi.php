...
class CTambahUsulanTransportasi extends MainPageD {       
                   ...
                   $no_rek5=addslashes($this->request['id']);
                    ...
                    $r=$this->DB->getRecord($str); 
                     if (isset($r[1])) {
                        ...
                        $this->lblAddKodeNamaUsulanTransportasi->Text=$data['no_rek5'].' / '.$data['nama_rek5'];
                        $this->lblAddNamaMerek->Text=$data['merek'];                          
                        $this->lblAddSatuan->Text=$this->DMaster->getNamaSatuan($data['id_satuan']);                       
                     }else{
                        throw new Exception ("Tidak bisa tambah Usulan karena No. Rekening Belum terdaftar.");
                     }
                }else{
                    $this->idProcess='view'; 
    ...
    public function saveData ($sender,$param) {
        if ($this->IsValid) {
            ...
            $txtHargaPesawatKelas=str_replace(',','',$this->txtAddHargaPesawatKelas->Text);
            $txtHargaPesawatEkonomi=str_replace(',','',$this->txtAddHargaPesawatEkonomi->Text);
            $txtHargaKapalKelas=str_replace(',','',$this->txtAddHargaKapalKelas->Text);
            $txtHargaKapalEkonomi=str_replace(',','',$this->txtAddHargaKapalEkonomi->Text);
            $ta=$_SESSION['ta'];
            
            $str = "INSERT INTO usulan SET idunit=$idunit,rekening='$no_rekening',pesawat_kelas='$txtHargaPesawatKelas',pesawat_ekonomi='$txtHargaPesawatEkonomi',kapal_kelas='$txtHargaKapalKelas',kapal_ekonomi='$txtHargaKapalEkonomi',ta=$ta,date_added=NOW(),date_modified=NOW()";
            $this->DB->insertRecord($str);
            
            $this->Pengguna->insertNewActivity($this->Page->getPagePath(),"Menambah Usulan transportasi baru dengan Rekening  ($no_rekening) telah berhasil dilakukan");
            
            $this->redirect('data.TambahUsulanTransportasi', true);
        }
    }
    public function lanjutInput($sender,$param) {
        if ($this->IsValid) {
            $no_rek5=$sender->Text;
            $this->redirect('data.TambahUsulanTransportasi', true,array('id'=>$no_rek5));   
        }
    }
}